...
@Repository
    ...
    @Query(value= "SELECT DISTINCT * FROM logements WHERE status=0 and ville LIKE %?1 OR  prix LIKE %?1 OR  type LIKE %?1 OR quartier LIKE %?1 OR piece LIKE %?1 " ,
            nativeQuery =true )
    List<Logement> findLogementsByCarectiristique(String mot_cle);

    @Query(value = "SELECT * FROM logements WHERE status=0 and type=?1 AND ville=?2 and quartier=?3 AND piece=?4 and prix BETWEEN ?5 and ?6" ,
    nativeQuery = true)
